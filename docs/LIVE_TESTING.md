...
| Model attention | Call `irc.attention.open` with one task channel. Merge its `filterAddition` with status/MOTD/protocol/state/DCC and other client requirements under `params.notifications` in one `subscriptions/listen` request. Send an unmentioned task-channel message, an account-tagged human message in `#control`, a nickname mention, and unrelated guest traffic. Run `irc.attention.check` from `initial_cursor` with `set_activity_anchor: true`, then repeat it after only unrelated traffic. Exercise a stale cursor and a gateway restart. | The listen stream receives matching watch updates plus requested lifecycle resource updates; no separate stream is opened per watch. The three relevant messages arrive once with `source_account` preserved, unrelated traffic does not. A drained quiet check omits both `events` and the redundant `activity` hint, advances `resume_cursor` to the journal head, and leaves the next ordinary tool hint quiet; gap/reset remains explicit. Claude/Codex fallback checks use `wait_ms: 0` immediately and with no interval longer than 60 seconds, while a direct host bridge invokes no model while quiet. |
